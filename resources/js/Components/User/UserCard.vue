...
<template>
    <div class="w-full max-w-sm main-border rounded-lg dark1:bg-gray-800 dark1:border-gray-700">
        <div class="flex flex-col items-center py-5 px-4 space-y-3">
            <img class="w-24 h-24 rounded-full shadow-lg" :src="userData.profileImgUrl"
                :alt="userData.first_name" />
            <h5 class=" text-xl font-medium text-gray-900 dark1:text-white">{{ userData.first_name }} {{ userData.last_name }}</h5>
            <span class="text-sm text-gray-500 text-center dark1:text-gray-400"> {{ userData.bio }}</span>
            <div class="flex">
                <button v-if="!userData.is_my_friend"
                    @click.prevent="toggleFriendRequest" 
                    ...
                    {{ (userData.friend_request) ? userData.friend_request.status : 'Add Friend' }}
